...
import { IsString, IsBoolean, IsOptional, IsInt, Matches, MinLength, MaxLength } from 'class-validator';
...
export class ProjectStatusDto {
  ...
  @ApiProperty({ description: 'File existence map', additionalProperties: { type: 'boolean' } })
...
export class InitProjectDto {
  ...
  @MaxLength(64)
  @Matches(/^[a-zA-Z0-9_-]+$/, {
    message: 'Project name can only contain alphanumeric characters, underscores, and hyphens',
  })
  name: string;
}
