...
from orchestrator.fixer.diagnosis import DiagnosisConfidence, RootCause
...
def test_diagnose_success(diagnosis_engine, mock_agent):
    ...
    async def run_test():
        result = await diagnosis_engine.diagnose(
            error=error, category=ErrorCategory.TYPE_ERROR, affected_files=[], context={}
        )
...
def test_diagnose_parsing_failure(diagnosis_engine, mock_agent):
    ...
    error = FixerError(error_id="e2", error_type="Unknown", message="fail", source="test")
