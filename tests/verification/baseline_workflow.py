...
class StateTracer:
    ...
    def _record(self, event_type: str, node_name: str, state: dict):
        ...
        snapshot = {
            ...
            "phase_status": {
                k: v.status.value if hasattr(v, "status") else v.status
...
async def run_baseline():
    ...
    with ExitStack() as stack:
        ...
        MockRunnerPlanning = stack.enter_context(
            patch("orchestrator.langgraph.nodes.planning.SpecialistRunner")
        )
        MockCursorAgent = stack.enter_context(patch("orchestrator.agents.CursorAgent"))
        MockSubprocessValidation = stack.enter_context(
            patch("orchestrator.langgraph.nodes.validation.asyncio.create_subprocess_exec")
        )
        MockSubprocessResearch = stack.enter_context(
            patch("orchestrator.langgraph.nodes.research_phase.asyncio.create_subprocess_exec")
        )
        MockRunnerImpl = stack.enter_context(
            patch("orchestrator.langgraph.nodes.task.nodes.SpecialistRunner")
        )

        # Patch Configs
        stack.enter_context(
            patch("orchestrator.langgraph.nodes.task.modes.USE_RALPH_LOOP", "false")
        )
        stack.enter_context(
            patch("orchestrator.langgraph.nodes.task.modes.USE_UNIFIED_LOOP_ENV", False)
        )

        # Patch Verification
        MockVerifyFiles = stack.enter_context(
            patch("orchestrator.langgraph.nodes.verify_task._verify_files_created")
        )
        MockRunTests = stack.enter_context(
            patch("orchestrator.langgraph.nodes.verify_task._run_task_tests")
        )

        # Patch Repositories
        MockLogsRepo = stack.enter_context(
            patch("orchestrator.db.repositories.logs.get_logs_repository")
        )
        MockPhaseRepo = stack.enter_context(
            patch("orchestrator.db.repositories.phase_outputs.get_phase_output_repository")
        )
        ...
        agent_impl.run.return_value = MagicMock(
            success=True, output=json.dumps(MOCK_IMPLEMENTATION_RESULT)
        )
        ...
        async with runner:
            try:
                await runner.run(progress_callback=tracer, config={"execution_mode": "afk"})
